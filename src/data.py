...
def add_test_rul(
    ...
    """Add ``actual_rul`` and ``clipped_rul`` to the test frame.
    ...
    ``clip_test_labels`` controls whether the label is also clipped (see config).
    """
    df = df_test.copy()
    max_cycle = df.groupby("unit_number")["time_cycles"].transform("max")
    remaining = df["unit_number"].map(rul_truth)
    df["actual_rul"] = remaining + (max_cycle - df["time_cycles"])
    if config.clip_test_labels:
        df["clipped_rul"] = df["actual_rul"].clip(upper=config.max_rul)
    else:
        df["clipped_rul"] = df["actual_rul"]
    return df
...
def make_test_last_windows(
    ...
    """One window per test unit: the last ``window_size`` cycles.
    ...
    left-padded by repeating the first cycle when ``pad_short`` is True.
    """
    channels = list(sensor_columns)
    windows, labels, win_units = [], [], []
